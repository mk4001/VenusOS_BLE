...
class Device(abc.ABC):
    ...
    def decrypt(self, data: bytes) -> bytes:
        ...
        initial_counter = int(container.iv).to_bytes(16, byteorder="little")
        cipher = Cipher(
            algorithms.AES(advertisement_key),
            modes.CTR(initial_counter),
        )
        decryptor = cipher.decryptor()
        return decryptor.update(container.encrypted_data[1:]) + decryptor.finalize()
